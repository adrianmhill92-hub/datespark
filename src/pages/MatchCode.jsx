...
export default function MatchCode() {
  ...
  async function handleSubmit(e) {
    e.preventDefault()
    if (!form.name || !form.city || form.interests.length === 0 || !form.vibe) {
      setError('Please fill in all required fields and pick at least one interest.')
      return
    }
    ...
    navigate(`/schedule/${dateCode}/${partnerCode}`)
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-rose-50 to-pink-100 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-lg w-full max-w-lg p-8">
        ...
        <p className="text-gray-500 mb-6 text-sm">
          Your match sent you a DateSpark link. Fill in your profile to see your date ideas.
        </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          {/* Vibe */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Date vibe *</label>
            <div className="flex flex-wrap gap-2">
              {VIBES.map(v => (
                <button
                  key={v}
          ...
          {/* Partner code — pre-filled from URL, read-only */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Your match's code</label>
            <div className="w-full bg-rose-50 border-2 border-rose-200 rounded-xl px-4 py-3 text-center">
              <span className="text-2xl font-mono font-bold tracking-[0.25em] text-rose-500">
                {partnerCode?.toUpperCase()}
              </span>
            </div>
            <p className="text-xs text-gray-400 mt-1">Pre-filled from your match's link.</p>
          </div>
          ...
          >
            {loading ? 'Saving…' : 'Spark our date ideas →'}
          </button>
        </form>
      </div>
    </div>
